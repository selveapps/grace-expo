...
import { NT_GROUPS } from '../../data/content';
import { colors, fonts, radius } from '../../theme';
import { readingTheme } from '../../readingTheme';

export default function NewTestamentScreen({ navigation }) {
  ...
  return (
    <Screen bg={RT.bg} edges={['top']} style={{ paddingHorizontal: 0 }}>
      <View style={styles.header}>
        ...
        <Text style={[styles.h1, { color: RT.ink }]}>New Testament</Text>
      ...
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        <BookAccordion groups={NT_GROUPS} onSelectBook={(b) => navigation.navigate('Book', { book: b })} />
      </ScrollView>
    </Screen>
  );
}
