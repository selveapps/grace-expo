/**
 * Book metadata for KJV seed validation (mirrors src/data/bookMeta.js CH map).
 */
...
export function testamentFor(book: string): 'old' | 'new' {
  return NT_BOOKS.includes(book) ? 'new' : 'old';
}

/** Chapters fetched in --sample mode for fast E2E (covers onboarding + reader paths). */
export const SAMPLE_CHAPTERS: { book: string; chapter: number }[] = [
