...
export type StoryMeta = {
  ...
  isPremium: boolean;
  parts: number;
  /** Optional hosted MP3 (supports `{storyId}` + `{part}` tokens). When set, skips TTS. */
  audioUrl?: string | null;
  /** Optional per-story ElevenLabs voice id override (see voiceProfiles.pickVoice). */
  voice?: string | null;
};

/** Default template: pre-rendered static MP3 per story part, served by /audio/:filename. */
const STORY_AUDIO_URL = '/audio/{storyId}-part-{part}.mp3';
...
export const STORIES: StoryMeta[] = [
  { id: 'ruth-stays', title: 'Ruth stays', subtitle: 'Loyalty in the harvest', hook: 'She had every reason to leave. She stayed.', scriptureRange: 'Ruth 1–4', testament: 'old', books: ['Ruth'], durationSeconds: 190, coverTint: '#6B5D4E', tags: ['Women of the Bible', 'Loyalty'], isPremium: true, parts: 4, audioUrl: STORY_AUDIO_URL, voice: STORY_VOICE['ruth-stays'] },
  { id: 'esther-uninvited', title: 'Esther walks in uninvited', subtitle: 'Courage in the palace', hook: 'For such a time as this.', scriptureRange: 'Esther 3–7', testament: 'old', books: ['Esther'], durationSeconds: 203, coverTint: '#5A4632', tags: ['Women of the Bible', 'Courage'], isPremium: true, parts: 4, audioUrl: STORY_AUDIO_URL, voice: STORY_VOICE['esther-uninvited'] },
  { id: 'davids-rooftop', title: "David's rooftop era", subtitle: 'A king, a mistake, a mercy', hook: 'Power looked away. Grace did not.', scriptureRange: '2 Samuel 11–12', testament: 'old', books: ['2 Samuel'], durationSeconds: 201, coverTint: '#4A382C', tags: ['Kings', 'Mercy'], isPremium: true, parts: 3, audioUrl: STORY_AUDIO_URL, voice: STORY_VOICE['davids-rooftop'] },
  { id: 'hannah-prayer', title: 'Hannah', subtitle: 'A prayer answered', hook: 'She prayed so hard they thought her drunk.', scriptureRange: '1 Samuel 1–2', testament: 'old', books: ['1 Samuel'], durationSeconds: 211, coverTint: '#6B5D4E', tags: ['Women of the Bible', 'Prayer'], isPremium: true, parts: 2, audioUrl: STORY_AUDIO_URL, voice: STORY_VOICE['hannah-prayer'] },
  { id: 'mary-annunciation', title: 'Mary', subtitle: 'The annunciation', hook: 'She said yes to the impossible.', scriptureRange: 'Luke 1', testament: 'new', books: ['Luke'], durationSeconds: 197, coverTint: '#5A4632', tags: ['Women of the Bible', 'Faith'], isPremium: false, parts: 3, audioUrl: STORY_AUDIO_URL, voice: STORY_VOICE['mary-annunciation'] },
];
...
export function resolveStoryAudioUrl(story: StoryMeta, part = 1): string | null {
  ...
  return story.audioUrl.replace('{storyId}', story.id).replace('{part}', String(part));
}

export function storyForClient(story: StoryMeta) {
  // Keep the `{part}` token so the app can resolve the right MP3 per part
  // (AudioService.resolveAudioUri fills `{part}` at play time).
  const audioUrl = story.audioUrl ? story.audioUrl.replace('{storyId}', story.id) : null;
  return {
    ...story,
